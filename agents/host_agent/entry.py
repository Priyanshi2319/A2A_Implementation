...
def main(host: str, port: int, registry: str):
    ...
    skill = AgentSkill(
        ...
        description=(
            "Routes user requests to the either portfolio agent or validator agent based on intent portfolio account list or validator report or summary"
        ),
        tags=["routing", "orchestration"],       # Keywords to aid discovery
        examples=[                                  # Sample user queries
            "portofolio account lists","what is the portofolio accounts",
            "validator Report","validation summary","validator status"
        ]
    )
